...
- Data stored with `pluginService.savePluginState` / `loadPluginState` under plugin id `dankBarTodo` (**todos**, **showCompleted**). Other bar instances update when DMS emits **`pluginStateChanged`** and the persisted snapshot differs from the local list (avoids clearing undo/menus after this instance’s own save).
