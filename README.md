...
- Outstanding count badge on the checklist icon (counts **New** and **In progress** only — not **Complete** or **Cancelled**).
- Status on the leading icon (order left → right): **Cancelled** (`cancel` icon, **`Theme.error`**) → **New** (hollow circle) → **In progress** (`play_circle`, blue) → **Complete** (`check_circle`, green). **Left click** steps one step toward **Complete** (no wrap: at **Complete**, left does nothing). **Right click** steps one step toward **Cancelled** (no wrap: at **Cancelled**, right does nothing). New todos default to **New**. Persisted data uses **`statusSchema`** **2**; older lists are migrated once from the previous three-state encoding.
